...
jouzu-adapter.cjs      the adapter (adapter-v1 over stdio)
extensions/            agent-presets, plan
presets/               preset definitions (standard, heavy-review)
```

* runtime: `jouzu@0.1.10` → `node runtime/dist/cli.js` (materialised under `runtime/`, not committed)
...
| `jouzu-adapter.cjs` | the hub spawns it (`command`) and speaks `adapter-v1` with it |
...
```
POST /v1/hub/plugins/jouzu/prepare        # asks this adapter; answers {ready, package, version, target, detail}
```
...
A hub started against a plugins directory that contains this one lists `jouzu` in
